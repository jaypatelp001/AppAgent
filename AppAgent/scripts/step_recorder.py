...
while True:
    ...
    labeled_img = draw_bbox_multi(screenshot_path, os.path.join(labeled_ss_dir, f"{demo_name}_{step}.png"), elem_list, True)

    # Calculate the aspect ratio of the original image
    original_height, original_width, _ = labeled_img.shape
    aspect_ratio = original_width / original_height

    # Define the desired width of the displayed image
    desired_width = 650   # to adjust the screne size you can change this value (screenshot size)

    # Calculate the corresponding height based on the aspect ratio
    desired_height = int(desired_width / aspect_ratio)

    # Resize the labeled image to the desired dimensions
    resized_img = cv2.resize(labeled_img, (desired_width, desired_height))
